...
rbt rbt_new_node(char* key){
	...
	rbt node = emalloc( sizeof *node ); //confused on how this works scince we are assigning 
...
int is_left_son(rbt son){
	...
	if(parent != NULL){
		...
		printf("node is root, cannot check if it is left son\n");
		return 0;
	}
...
void rbt_print_preorder(rbt t){
	
/**print a BST recursibly =D*/

	if(t != NULL) rbt_print_node(t);

	if(t->left != NULL) rbt_print_preorder(t->left);
			//go left	
	
	if(t->right != NULL) rbt_print_preorder(t->right);
			//go right
...
rbt rbt_get(rbt t, char* key){
	...
	fprintf(stderr, "somthing when wrong when trying to get node\n");
	return NULL;
}
...
rbt black_uncle_roations(rbt son, rbt parent, rbt grandp){
	...
	printf("son %s, parent %s, grandp %s\n", son->key, parent->key, grandp->key);
		...
		if(is_left_son(parent)){ //left childs
			is_leftchilds = 1;
			if(is_left_son(son)){
				//is outtterchild
				printf("left side, outterchild %s\n", son->key);
				is_innerchild = 0;
			}else{
				//is inner child
				printf("left side, innerchild %s\n", son->key);
				is_innerchild = 1;
			}
		}else{	//right childs
			is_leftchilds = 0;
			if(is_left_son(son)){ 
				//is inner child
				printf("right side, innerchild %s\n", son->key);
				is_innerchild = 1;
			}else{
				//is outerchild
				is_innerchild = 0;
				printf("right side, outerchild %s\n", son->key);
			}
		}	
...
int rb_check(rbt t){
	//*checks the tree for an ocurance of two red nodes*/ 	
	...
	if(ISRED(t) && ISRED(t->parent)){
		...
		if(is_uncle_red(t)){ //uncle is red
...
/**append to node the the binary search tree return the new ponter to the rbt */
	rbt node;
	...
	rbt_print_inorder(node);
	//rbt_printg(node);
	while(rb_check(node) != 1){
		printf("REDOING red black check");
		printf("node: %s\n", key);
		node =  get_root(node);
		printf("\n--------------------------\n\n");
	}
...
int _print_t(rbt tree, int is_left, int offset, int depth, char s[20][255])
{
    char b[20];
    int width = 5;
    ...
    if( ISRED(tree) ) sprintf(b, "(R%2s)", tree->key);
	else if (ISBLACK(tree)) sprintf(b, "(B%2s)", tree->key);
	else sprintf(b, "(%3s)", tree->key);
